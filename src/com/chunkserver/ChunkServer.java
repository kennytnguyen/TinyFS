...
public class ChunkServer implements ChunkServerInterface {
	...
	public static int PayloadSZ = Integer.SIZE / Byte.SIZE; // Number of bytes
															// in an integer
	public static int CMDlength = Integer.SIZE / Byte.SIZE; // Number of bytes
															// in an integer
	...
	public ChunkServer() {
		...
		if (mySocket == null) {
			try {
				...
				oos.writeObject(Constants.ChunkServerTag);
	...
	/**
	 * Each chunk is corresponding to a file. Return the chunk handle of the
	...
	/**
	 * Write the byte array to the chunk at the offset The byte array size
	 * should be no greater than 4KB
	 */
	public boolean writeChunk(String ChunkHandle, byte[] payload, int offset) {
		try {
			// If the file corresponding to ChunkHandle does not exist then
	...
	public static void ReadAndProcessRequests() {
		...
		int ServerPort = 0; // Set to 0 to cause ServerSocket to allocate the
							// port
		...
		try {
			// Allocate a port and write it to the config file for the Client to
			// consume
		...
		while (!done) {
			try {
				...
				// Use the existing input and output stream as long as the
				// client is connected
				while (!ClientConnection.isClosed()) {
					...
					case WriteChunkCMD:
						...
						chunkhandlesize = payloadsize - ChunkServer.PayloadSZ - ChunkServer.CMDlength - (2 * 4)
								- payloadlength;
						if (chunkhandlesize < 0)
